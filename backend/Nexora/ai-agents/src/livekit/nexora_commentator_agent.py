#!/usr/bin/env python3
"""
Nexora AI Battle Commentator Agent using LiveKit Agents Framework
...
class NexoraBattleCommentator(VoiceAssistant):
    """Nexora AI Battle Commentator using LiveKit Agents framework"""
    ...
    async def on_agent_start(self, ctx: JobContext):
        ...
        print("🎙️ Nexora Battle Commentator started!")
        
        # Initialize battle context if we have project info
        project_id = ctx.room.name.replace("nexora-battle-", "")
        if project_id:
...
async def entrypoint(ctx: JobContext):
    ...
    commentator = NexoraBattleCommentator(ctx, options)
