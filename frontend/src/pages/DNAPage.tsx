...
function TopicBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-[9px] font-black uppercase tracking-widest text-white/60 w-28 shrink-0 truncate">
        {topic}
      </span>
...
// ─── Blind Spot Card ─────────────────────────────────────────────────────────
const BLIND_SPOT_CONTEXT: Record<
  string,
  { why: string; risk: string; color: string }
> = {
  RBI: {
    why: "RBI policy decisions affect your EMIs, bond yields, and banking stocks.",
    risk: "High Risk",
    color: "#ED1C24",
  },
  Markets: {
    why: "Ignoring market trends can cost you entry/exit timing.",
    risk: "High Risk",
    color: "#ED1C24",
  },
  "Mutual Funds": {
    why: "MF regulations and category changes affect your SIP returns.",
    risk: "Medium Risk",
    color: "#FFD700",
  },
  Banking: {
    why: "Banking health signals credit cycles that impact all your holdings.",
    risk: "Medium Risk",
    color: "#FFD700",
  },
};
...
function BlindSpotCard({ topic }: { topic: string }) {
  const ctx = BLIND_SPOT_CONTEXT[topic] ?? {
    why: "Staying updated here improves your overall portfolio view.",
    risk: "Medium Risk",
    color: "#FFD700",
  };
  return (
      ...
      <div className="flex flex-col gap-2 flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          <span
            ...
            style={{ color: ctx.color, borderColor: `${ctx.color}40` }}
          >
            {ctx.risk}
          </span>
        </div>
  ...
  return (
    <div className="bg-[#0a0a0a] border border-white/10 rounded-xl p-6 flex flex-col gap-4 h-full">
      <div className="flex items-center gap-3 border-b border-white/10 pb-4">
        ...
        <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-[#FFD700]">
          Just For You
        </h3>
      ...
      {loading ? (
        ...
        <p className="text-sm text-white/80 leading-relaxed font-medium italic flex-1">
          "{insight}"
        </p>
      )}
        ...
        <div className="mt-2">
          ...
          <div className="flex flex-col gap-2">
            {profile.blindSpots.slice(0, 3).map((topic, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-white/5 hover:border-[#ED1C24]/30 transition-all cursor-pointer group"
              >
    ...
    <div className="flex-1 flex flex-col items-center justify-center gap-8 p-8">
      <div className="text-center max-w-sm">
        ...
        <p className="text-sm text-white/50 leading-relaxed mb-6">
          Read {steps - sessions} more{" "}
          {steps - sessions === 1 ? "article" : "articles"} to unlock your
          personalized News DNA profile.
        </p>
        <div className="bg-[#0a0a0a] border border-white/10 rounded-xl p-6 mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-[10px] font-black text-[#FFD700]">
              {sessions}/{steps} Reads
            </span>
          </div>
          <div className="h-2 bg-white/5 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[#ED1C24] to-[#FFD700] rounded-full transition-all duration-700"
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
...
export default function DNAPage() {
  ...
  return (
    ...
    >
      {isOnboarding ? (
        <OnboardingState sessions={profile.totalSessions} />
      ) : (
        <main
          className="flex-1 min-h-0 overflow-y-auto lg:overflow-hidden p-2 sm:p-3 md:p-4 lg:p-5 gap-3 sm:gap-4 flex flex-col"
          style={{
            scrollbarWidth: "thin",
            scrollbarColor: "rgba(237,28,36,0.3) transparent",
          }}
        >
          {/* Hero Section */}
          <section className="bg-[#121212] border border-white/10 rounded-xl p-4 sm:p-6 flex flex-col lg:flex-row items-start lg:items-center gap-5 sm:gap-6 relative overflow-hidden shrink-0">
            ...
            <div className="flex-1 z-10">
              ...
              <h2 className="text-2xl sm:text-3xl xl:text-4xl 2xl:text-5xl font-black italic uppercase tracking-tighter text-white leading-none mb-1">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-white/30 mb-3">
                Built from {profile.totalSessions} reading{" "}
                {profile.totalSessions === 1 ? "session" : "sessions"}
              </p>
              <p className="text-sm text-white/60 max-w-lg leading-relaxed">
                {profile.investorDescription}
              </p>
            </div>
            <div className="z-10 flex flex-col items-start lg:items-end gap-2 w-full lg:w-auto">
              <div className="bg-[#ED1C24] text-white px-4 py-2 rounded-xl font-black uppercase tracking-widest text-[10px] sm:text-[11px] shadow-[0_0_20px_rgba(237,28,36,0.4)]">
          ...
          {/* 3-Column Grid */}
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 min-h-0 xl:flex-1 xl:overflow-hidden">
            {/* Column 1 — Reading Traits */}
            <div
              className="bg-[#0a0a0a] border border-white/10 rounded-xl p-5 sm:p-6 flex flex-col gap-5 overflow-y-auto min-h-[320px]"
                ...
                <div className="flex flex-col gap-3">
                  {profile.traits.map((t) => (
                    <TopicBar
                      ...
                      topic={t.topic}
                      score={t.score}
                      isBlindSpot={profile.blindSpots.includes(t.topic)}
                    />
                  ))}
                </div>
              ) : (
                ...
                <>
                  ...
                  <div className="flex flex-col gap-2">
                    {profile.emotionalTriggers.map((trig, i) => (
                      ...
                      >
                        <span className="text-[10px] font-bold text-white/70 truncate mr-2">
                          {trig.topic}
                        </span>
                        <span className="text-[8px] font-black uppercase tracking-widest text-[#FFD700] shrink-0">
                          {trig.behavior}
                        </span>
                      </div>
                    ))}
            ...
            >
              <div className="flex items-center gap-3 border-b border-white/10 pb-4 shrink-0">
                ...
                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-[#ED1C24]">
                  What You're Missing
                </h3>
                <span className="ml-auto text-[9px] font-black text-white/30">
                  {profile.blindSpots.length} detected
                </span>
              </div>
              {profile.blindSpots.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center flex-1 gap-3">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/40 text-center">
                    You're reading it all. Nothing is missing.
                  </p>
                </div>
              )}
            </div>
          ...
          {/* Footer Bar */}
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-4 py-3 bg-[#0a0a0a] border border-white/10 rounded-xl shrink-0">
            <p className="text-[9px] font-black uppercase tracking-widest text-white/30">
              Your DNA updates every time you read
            </p>
            <button
              onClick={() => navigate("/arena")}
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-[#ED1C24]/10 border border-[#ED1C24]/30 rounded-lg text-[9px] font-black uppercase tracking-widest text-[#ED1C24] hover:bg-[#ED1C24] hover:text-white transition-all"
            >
